  ...
  describe 'class methods' do
    it '.munchies_at_destination' do
      VCR.use_cassette('munchies-data') do
        munchie = MunchieFacade.munchies_at_destination('denver,co', 'pueblo,co', 'chinese')
    ...
    it '.munchies_details' do
      VCR.use_cassette('munchies-data') do
        data = MunchieFacade.munchies_details('denver,co', 'pueblo,co', 'chinese')
    ...
    it '.restaurant' do
      VCR.use_cassette('yelp-business-search') do
        restaurant = MunchieFacade.restaurant('denver,co', 1611021600, 'chinese')
